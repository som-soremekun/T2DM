...
View(pheno)
```
#### The data contains 11 columns and 6407 rows (study participants). The colums include the individual ids, hba1c values and covariates.

### View the summary of the column in the dataset
#### Hba1c values for the individuals are to be converted to either cases or control based on their value. Therefore, it is important to get an idea of the column distribution. From the result, the minimum hba1c value is 2.7 while the maximum is 20.6. The value for 7 participants are missing.
```{r}
...
#### Alternatively, the "sex" column can be changed directly to replace males with 1 and females with 0.
```{r}
pheno$sex <- ifelse(pheno$sex=="Male",1,0)
```

#### Alternatively, both commands can be run together with the code below
```{r}
pheno <- pheno %>%
	mutate(t2d = case_when(
		hba1c2 < 6.5 ~ 0,
