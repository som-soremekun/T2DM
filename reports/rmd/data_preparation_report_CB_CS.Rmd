...
This report describes the phenotype data transformation for the genome-wide association of T2D in UGR.  
...
The data contains `r ncol(pheno)` columns and `r nrow(pheno)` rows study participants. The colums include the individual ids, hba1c values and the covariates.
...
The data contains `r pheno %>% group_by(sex) %>% tally()` females and males.
...
hist(pheno$height, main ="Height",
     ...
     freq = TRUE)
```
This shows the summary of the height distribution in the population. The minimum height is `r min(pheno$height)`, while the maximum height is `r max(pheno$height)`. The values for `r pheno %>% filter(is.na(height)) %>% nrow()` individuals are not available.
...
This shows the age distribution summary of the participants. The minimum age is `r min(pheno$age)`, while the maximum age is `r max(pheno$age)`.
...
This shows the weight distribution summary of the participants. The minimum weight is `r min(pheno$weight)`, while the maximum weight is `r max(pheno$weight)`.The histogram plot shows the weight distribution in the dataset. 
...
This shows the bmi distribution summary of the participants. The minimum bmi is `r min(pheno$bmi)`, while the maximum bmi is `r max(pheno$bmi)`. The histogram plot shows the bmi distribution of the study participants
...
Hba1c values for the individuals are to be converted to either cases or control based on their value. Therefore, it is important to get an idea of the column distribution. From the result, the minimum hba1c value is `r min(pheno$hba1c2)` while the maximum is `r max(pheno$hba1c2)`. The value for `r pheno %>% filter(is.na(hba1c2)) %>% nrow()` participants are missing.
The plot gives a graphical view of hba1c distribution.
...
table(pheno$t2d)
```
The data contains `r pheno %>%  group_by(t2d) %>% tally()` diabetic individuals(cases) and non-diabetes individuals(controls)



## Step 2: Phenotype definition and transformation
...
## Step 3: GWAS Input File Preparation
